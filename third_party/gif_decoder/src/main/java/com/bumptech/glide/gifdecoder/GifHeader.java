...
public class GifHeader {
  ...
  @ColorInt int[] gct = null;
  @GifDecoder.GifDecodeStatus int status = GifDecoder.STATUS_OK;
  int frameCount = 0;
  ...
  boolean gctFlag;
  /**
   * Size of Global Color Table. The value is already computed to be a regular number, this field
   * doesn't store the exponent.
   */
  int gctSize;
  /** Background color index into the Global/Local color table. */
  int bgIndex;
  /**
   * Pixel aspect ratio. Factor used to compute an approximation of the aspect ratio of the pixel in
   * the original image.
   */
  int pixelAspect;
