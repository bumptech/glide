...
Clearing all memory isn't particularly efficient and should be avoided whenever possible to avoid jank and loading times if your application is opened again.

For a more moderate approach, you can call Glide using [``ComponentCallbacks2``][20] to automatically reduce Glide's memory usage incrementally when the Android system asks your application to do so.
...
Glide provides only limited controls for the disk cache size. To try to clear out all items in the disk cache, you can use [``clearDiskCache``][23]:
...
To change the amount of sdcard space available to Glide acorss your application, see the [Configuration page][24].
