...
public class GifDrawableTransformationTest {
  ...
  public void testSetsTransformationAsFrameTransformation() {
    ...
    when(
        wrapped.transform(
            any(Context.class), GifDrawableTransformationTest.<Bitmap>anyResource(), anyInt(),
            anyInt()))
        .thenReturn(expectedResource);

    transformation.transform(context, resource, width, height);

    verify(gifDrawable).setFrameTransformation(
        isA(Transformation.class), eq(expectedBitmap));
  }

  @SuppressWarnings("unchecked")
  private static <T> Resource<T> anyResource() {
    return any(Resource.class);
  }

  @SuppressWarnings("unchecked")
  private static <T> Resource<T> mockResource() {
    return mock(Resource.class);
  }

  @Test
  public void testEquals() throws NoSuchAlgorithmException {
    doAnswer(new WriteDigest("first")).when(wrapped)
