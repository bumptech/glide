...
import com.bumptech.glide.integration.gifdecoder.GifDrawableTest.BitmapTrackingShadowCanvas;
...
public class GifDrawableTest {
  ...
  public void testDoesDrawCurrentFrameIfOneIsAvailable() {
    ...
    drawable.draw(canvas);
    verify(canvas).drawBitmap(eq(currentFrame), (Rect) Matchers.isNull(),
        isARect(), isAPaint());
    verify(canvas, never()).drawBitmap(eq(firstFrame),
        (Rect) Matchers.isNull(), isARect(), isAPaint());
  }
  ...
  public void testDoesNotDrawFrameAfterRecycle() {
    ...
    Canvas canvas = mock(Canvas.class);
    drawable.draw(canvas);
    verify(canvas, never()).drawBitmap(eq(bitmap), isARect(), isARect(),
        isAPaint());
  }
  ...
  public void testSetsFrameTransformationOnFrameManager() {
    ...
    verify(frameLoader).setFrameTransformation(
        eq(transformation), eq(bitmap));
  }
  ...
  public void testAppliesGravityOnDrawAfterBoundsChange() {
    ...
    verify(canvas).drawBitmap(isA(Bitmap.class), (Rect) Matchers.isNull(),
        eq(bounds), eq(paint));
  }
  ...
  public void testReturnsCurrentTransformationInGetFrameTransformation() {
    ...
    verify(frameLoader).setFrameTransformation(
        eq(newTransformation), eq(bitmap));
  }
  ...
  private void verifyRanLoops(int loopCount, int frameCount) {
    ...
    verify(cb, times(1 + loopCount * frameCount))
        .invalidateDrawable(eq(drawable));
  }
