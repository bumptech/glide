...
/**
 * Start a request by passing [model] to [RequestBuilder.load] using the given [requestManager]
 * and then applying the [requestBuilderTransform] function to add options or apply mutations if
 * the caller desires.
 *
 ...
 *
 * If you want to restrict the size of this [Composable], use the given [modifier]. If you'd like
 * to force the size of the pixels you load to be different than the display area, use
 ...
 *
 * If you provide your own [requestManager] rather than using this method's default, consider
 * using [remember] at a higher level to avoid some amount of overhead of retrieving it each
 * re-composition.
 *
...
private fun rememberRequestBuilderWithDefaults(
  ...
  requestBuilderTransform(
    requestManager
      .load(model)
      .contentScaleTransform(contentScale)
  )
}
