...
internal data class LibraryGlideModuleData(
  val name: LibraryModuleName, val containingFile: KSFile?,
) {
    ...
    if (uniqueLibraryGlideModules != libraryGlideModules) {
      ...
      val duplicateModules: List<String> =
        allLibraryGlideModules
          .fold(mutableMapOf<String, Int>()) { acc, current ->
            val key = current.name.qualifiedName
            acc[key] = (acc[key] ?: 0) + 1
            acc
          }
          .filter { it.value > 1 }
          ...
          .toList()
      environment.logger
        .warn(GlideSymbolProcessorConstants.DUPLICATE_LIBRARY_MODULE_ERROR.format(duplicateModules))
    }
