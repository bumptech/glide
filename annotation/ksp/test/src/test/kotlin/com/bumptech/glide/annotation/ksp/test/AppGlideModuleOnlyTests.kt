...
class OnlyAppGlideModuleTests(override val sourceType: SourceType) : PerSourceTypeTest {
  ...
  companion object {
    @Parameterized.Parameters(name = "sourceType = {0}")
    @JvmStatic
    fun data() = SourceType.values()
  }

  @Test
  fun `with @GlideModule on non-library class, compilation fails`() {
    val kotlinSource =
      KotlinSourceFile(
        "Something.kt",
        """
  ...
  @Test
  fun `with @GlideModule on valid AppGlideModule, generates GeneratedAppGlideModule`() {
    val kotlinModule =
      KotlinSourceFile(
        "Module.kt",
        """
  ...
  @Test
  fun `with AppGlideModule constructor that accepts only Context, generates GeneratedAppGlideModule`() {
    val kotlinModule =
      KotlinSourceFile(
        "Module.kt",
        """
  ...
  @Test
  fun `with AppGlideModule constructor that accepts something other than a Context, fails`() {
    val kotlinModule =
      KotlinSourceFile(
        "Module.kt",
        """
  ...
  @Test
  fun `with AppGlideModule constructor that accepts multiple arguments, fails`() {
    val kotlinModule =
      KotlinSourceFile(
        "Module.kt",
        """
  ...
  @Test
  fun `with AppGlideModule with one constructor that accepts Context, one that's empty, uses the Context constructor`() {
    val kotlinModule =
      KotlinSourceFile(
        "Module.kt",
        """
  ...
  @Test
  fun `with multiple AppGlideModules, fails`() {
    val firstKtModule =
      KotlinSourceFile(
        "Module1.kt",
        """
