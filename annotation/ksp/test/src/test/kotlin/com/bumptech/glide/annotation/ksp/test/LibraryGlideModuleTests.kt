...
class LibraryGlideModuleTests(override val sourceType: SourceType) : PerSourceTypeTest {
  ...
  @Test
  fun `with @GlideModule on valid LibraryGlideModule, succeeds but does not generate GeneratedAppGlideModule`() {
    val kotlinModule =
      KotlinSourceFile(
        "Module.kt",
        """
  ...
  @Test
  fun `with valid LibraryGlideModule, and AppGlideModule, calls LibraryGlideModule from GeneratedAppGlideModule`() {
    ...
    compileCurrentSourceType(
      kotlinAppModule, kotlinLibraryModule, javaAppModule, javaLibraryModule
    ) {
      assertThat(it.exitCode).isEqualTo(ExitCode.OK)
  ...
  @Test
  fun `with multiple LibraryGlideModules, and AppGlideModule, calls LibraryGlideModules from GeneratedAppGlideModule`() {
  ...
  @Test
  fun `with the same LibraryGlideModule in multiple files, and AppGlideModule, calls LibraryGlideModule once from GeneratedAppGlideModule`() {
  ...
  @Test
  fun `with LibraryGlideModules with different packages but same name in multiple files and AppGlideModule, calls LibraryGlideModule once from GeneratedAppGlideModule`() {
