...
/**
 * An interface that allows a transition to be applied to {@link android.view.View}s in
 * {@link com.bumptech.glide.request.target.Target}s in across resource types. Targets that wrap views will be able to
 * provide all of the necessary arguments and start the transition. Those that do not will be unable to provide the
 * necessary arguments and will therefore be forced to ignore the transition. This interface is a compromise that
 * allows view specific transition in Glide's complex world of arbitrary resource types and arbitrary target types.
 *
...
public interface Transition<R> {
     ...
     * as transitions: ({@link ViewTransition}, {@link ViewPropertyTransition} and animated
     ...
     * @param adapter The {@link Transition.ViewAdapter} wrapping a view that
     *                can at least return an {@link android.view.View} from
     *                {@link Transition.ViewAdapter#getView()}.
     * @return True if in the process of running the transition, the new resource was set on the view, false if the
     * caller needs to manually set the current resource on the view.
     */
