 ...
 * #loadData(Priority)} may or may not be called for any given load depending on whether or not the
 * corresponding resource is cached. Cancel also may or may not be called. If {@link
 * #loadData(Priority)} is called, then so {@link #cleanup()} will be called.
 *
...
public interface DataFetcher<T> {
   ...
   * finally block after the data returned by {@link #loadData(Priority)} has been decoded by the
