...
public class SourceResourceRunnerTest {
    ...
    public void testEncoderIsCalledWithTransformedIfOriginalResourceInCache() {
        ...
        when(harness.cacheLoader.load(eq(harness.originalKey), eq(harness.cacheDecoder),
                eq(harness.width), eq(harness.height))).thenReturn(harness.decoded);
    ...
    public void testPriorityMatchesPriority() {
        for (Priority priority : Priority.values()) {
            ...
            assertEquals(priority.ordinal(), actualPriority);
        }
    }
    ...
    public void testNotifiesJobOfFailureIfTransformationThrows() throws Exception {
        harness.mockSuccessfulFetchAndDecode();
        Exception exception = new RuntimeException("test");
        when(harness.transformation.transform(any(Resource.class), anyInt(), anyInt())).thenThrow(exception);
    ...
    public void testNotifiesJobOfFailureIfDiskCacheThrows() throws Exception {
        harness.mockSuccessfulFetchAndDecode();
        Exception exception = new RuntimeException("test");
        doThrow(exception).when(harness.diskCache).put(any(Key.class), any(DiskCache.Writer.class));
    ...
    public void testNotifiesJobOfFailureIfFetcherThrows() throws Exception {
        ...
        when(harness.fetcher.loadData(any(Priority.class))).thenThrow(exception);
    ...
    public void testNotifiesJobOfFailureIfDecoderThrows() throws Exception {
        ...
        Exception exception = new RuntimeException("test");
        when(harness.decoder.decode(anyObject(), anyInt(), anyInt())).thenThrow(exception);
    ...
    private static class SourceResourceHarness {
        ...
        SourceResourceRunner.SourceWriter<Object> sourceWriter = mock(SourceResourceRunner.SourceWriter.class);
